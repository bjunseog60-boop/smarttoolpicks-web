  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="space-y-6">
        {filtered.map((p, i) => (
          <div key={`${p.name}-${i}`} className="flex flex-col md:flex-row gap-6 p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 relative overflow-hidden">
            ...
            <div className="w-full md:w-48 h-32 flex-shrink-0 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-center p-4 relative mt-4 md:mt-0">
              <img
                src={p.image || 'https://placehold.co/400x300?text=Software+Logo'}
                alt={p.name}
            ...
            <div className="flex-1 flex flex-col justify-center">
              <div className="flex items-start justify-between gap-4">
                ...
                {p.tag && (
                  <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-green-100 text-green-800 uppercase tracking-wide whitespace-nowrap border border-green-200">
                    {p.tag}
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 text-sm bg-gray-50 p-3 rounded-lg border border-gray-100">
                ...
                <div>
                  <span className="block text-gray-500 text-xs uppercase tracking-wider mb-1">Best For</span>
                  <span className="font-medium text-gray-700">B2B / Professionals</span>
                </div>
              </div>
            </div>
            ...
            <div className="w-full md:w-48 flex flex-col justify-center border-t md:border-t-0 md:border-l border-gray-100 pt-5 md:pt-0 md:pl-6">
              <a
                ...
                className="w-full text-center px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-sm transition-colors"
              >
                Visit Website
              </a>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-gray-500">No software found for this category.</div>
        )}
      </div>
    </div>
  );
